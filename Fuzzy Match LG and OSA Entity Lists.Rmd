...
lg_registry <-
  ...
  filter(entity_name != "Test", entity_name != ",mnn") %>% 
  mutate(entity_name = str_to_title(entity_name))

lg_registry$exact_duplicate <- duplicated(lg_registry)
lg_registry$duplicate_name  <- duplicated(lg_registry$entity_name)
...
sf_data <- 
  list(
    ...
    govt_type    = sf_govt_type)

rm(sf_entities, sf_govt_type)
```
...
## Learning `fuzzyjoin::stringdist_join()`
...
* On initial import, 1707 observations of 38 variables.
  ...
  * Eight observations that are exact duplicates of at least one other observation (across all variables).
...
* After removing the two test entities, the entity named ",mnn.," and the eight observations that are exact duplicates of another observation, the LG Registry contains 1696 entities.


compared to the 1681 entities OSA 
